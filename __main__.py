...
def glyph_from_path(path: str) -> Glyph:
    ...
    svg = SVGPath.fromstring(
        ...
        Identity.translate(0, FONT_SIZE).scale(FONT_SIZE / ICON_SIZE,
                                               -FONT_SIZE / ICON_SIZE))
...
def tuple_variation(base: Glyph, variation: tuple[int, str]) -> TupleVariation:
    """Generate a tuple variation from the base glyph, variation coordinate, and variation path"""
    ...
    # If we provide points only for points that change, neighboring points also get interpolated,
    ...
    return TupleVariation(
        ...
        [(glyph.coordinates[i][0] - base.coordinates[i][0],
          glyph.coordinates[i][1] -
          base.coordinates[i][1]) if i in selected else None
...
def main():
    ...
    for symbol in SYMBOLS:
        ...
        char_map |= {
            code: symbol.name
            for code in symbol.characters if isinstance(code, int)
        }
        ...
        g_var |= {
            symbol.name:
            [tuple_variation(glyph, v) for v in symbol.variants.items()]
        }

        h_metrics |= {symbol.name: (FONT_SIZE, glyph.xMin)}

        # TODO: Ligatures aren't supported by fonttools yet --
